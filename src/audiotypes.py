...
class SongFile(ABC):
    ...
    def applyNameTranslations(self):
        try:
            for artist in self.artistNameTranslations:
                if self.getSongArtist() in artist["incorrectNameList"]:
                    self.setSongArtist(artist["correctName"])
                    return "Set song artist to found name translation: \"" + artist["correctName"] + "\""
                elif self.getSongArtist() == artist["correctName"]:
                    return "Song artist name translation found. Already in correct format: \"" + self.getSongArtist() + "\""
            return "No name translation found in input file for artist: \"" + self.getSongArtist() + "\""
        except KeyError:
            return ("ERROR: Failed parsing artistNameTranslations list, couldn't find a key value. " 
                "Key values are: incorrectNameList and correctName")
        except:
            return "ERROR: Unknown error occurred parsing artistNameTranslations list."
